...
export default Kapsule({
    ...
    update(state) {
        ...
        function renderTimelines(maxElems) {
            ...
            var segment = newSegments.append("svg")
              .attr('class', 'svgband')
              .attr('width', function (d) { return d3Max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]) })
            ...
            segment.append("text")
                     .text(function (d) { return state.timelineText ? d.description : "" })
                     .style("fill", "black")
            ...
            newSegments
                .on('mouseover', function () {
                    ...
                    select(this).transition().duration(70)
                           .attr('x', function (d) { return state.xScale(d.timeRange[0]) - hoverEnlarge / 2; })
                           .attr('width', function (d) { return d3Max([1, state.xScale(d.timeRange[1]) - state.xScale(d.timeRange[0])]) + hoverEnlarge; })
                           .attr('y', function (d) { return state.yScale(d.group + '+&+' + d.label) - (state.lineHeight + hoverEnlarge) / 2; })
