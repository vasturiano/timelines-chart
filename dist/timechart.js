...
(function (global, factory) {
  ...
  var legend = Kapsule({
      ...
      init: function init(el, state) {
          state.legend = select$1(el).append('g').attr('class', 'legend');
      },
      update: function update(state) {
  ...
  var TimeOverview = Kapsule({
    ...
    init: function init(el, state) {
      ...
      state.svg = select$1(el).append('svg').attr('class', 'brusher');
  ...
  var timelines = Kapsule({
      ...
      data: {
        ...
        onChange: function onChange(data, state) {
          ...
          function parseData(rawData) {
            ...
            var parseDate = timeParse(state.timeFormat);
            ...
            if (state.groupLayout) {
              var keys = state.completeFlatData.map(function (d) {
                return d.group;
              }).reduce(function (p, v) {
              ...
              keys.forEach(function (key) {
                var items = state.completeFlatData.filter(function (d) {
                  return d.group === key;
                ...
                var i,
                    line,
                    lines = [];
                var top = d3Max(state.completeFlatData, function (d) {
                  return d.line;
                }) || 0;
                items.forEach(function (item) {
                  for (i = 0, line = 0; i < lines.length; i++, line++) {
                    if (item.timeRange[1] <= lines[i]) {
                      break;
                    }
                  }
              ...
              state.completeStructData.sort(function (a, b) {
                return ascending(a.group, b.group);
              });
            } //restart here all layouts


            var gps = state.completeFlatData.map(function (d) {
              return d.group;
            }).reduce(function (p, v) {
              return p.indexOf(v) == -1 ? p.concat(v) : p;
            }, []);
            ...
            for (var i = 0, ilen = gps.length; i < ilen; i++) {
              state.completeStructData.push({
                group: gps[i],
                lines: state.completeFlatData.map(function (d) {
                  return d.group === gps[i] ? d.label : null;
                }).reduce(function (p, v) {
                  return p.indexOf(v) == -1 ? p.concat(v) : p;
                }, []).filter(function (d) {
                  return typeof d !== "undefined" ? d !== null : false;
            ...
            var totalLines = state.completeFlatData.map(function (d) {
              return d.label;
            }).reduce(function (p, v) {
              return p.indexOf(v) == -1 ? p.concat(v) : p;
            }, []).filter(function (d) {
            ...
            state.groupLayout ? state.totalNLines = d3Max(state.completeFlatData, function (d) {
              return d.line;
            }) + 1 : state.totalNLines = totalLines.length;
          }
        }
      },
      width: {
    ...
    methods: {
      ...
      getSvg: function getSvg(s) {
        return select$1(s.svg.node().parentNode).html();
      },
    ...
    init: function init(el, state) {
      var elem = select$1(el).attr('class', 'timelines-chart');
      ...
      function buildDomStructure() {
        ...
        function invertOrdinal(val, cmpFunc) {
          ...
          if (scRange.length === 2 && scDomain.length !== 2) {
            ...
            scRange = sequence(scRange[0], scRange[1], (scRange[1] - scRange[0]) / scDomain.length);
          }
        ...
        function addOverviewArea() {
          ...
          state.svg.on('zoomScent', function () {
            ...
            if (!state.overviewArea || !zoomX) return; // Out of overview bounds
      ...
      function addZoomSelection() {
        state.graph.on('mousedown', function () {
          if (select$1(window).on('mousemove.zoomRect') != null) // Selection already active
            return;
          var e = this;
          ...
          select$1(window).on('mousemove.zoomRect', function () {
            ...
            select$1(window).on('mousemove.zoomRect', null).on('mouseup.zoomRect', null);
            select$1('body').classed('stat-noselect', false);
            rect.remove();
    ...
    update: function update(state) {
      ...
      function renderAxises() {
        ...
        state.svg.select('g.x-grid').attr('transform', 'translate(0,' + state.graphH + ')').transition().duration(state.transDuration).call(state.xGrid); // Y
        ...
        state.yAxis.tickFormat(function (d) {
          return state.rightAxis ? reduceLabel(d.split('+&+')[1], maxChars) : "";
        });
      ...
      function renderTimelines(maxElems) {
        ...
        var newSegments = timelines.enter().append('g').attr('class', 'series-segment').attr("transform", function (d) {
          tmp = state.yScale(d.group + '+&+' + d.label) - state.lineHeight / 2;
          return "translate(" + state.xScale(d.timeRange[0]) + "," + tmp + ")";
        ...
        segment.append("text").text(function (d) {
          return state.timelineText ? d.description : "";
